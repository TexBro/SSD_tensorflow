...
"""
im = np.array(Image.open('./demo/3_003.jpg'), dtype=np.uint8)
...
for i in range(rbboxes.shape[0]):    
    ...
    width = int(bbox[2]*shape[0] - bbox[0]*shape[0])
    height = int(bbox[3]*shape[1] - bbox[1]*shape[1])
    rect=patches.Rectangle(p1,height,width,linewidth=1,edgecolor='r',facecolor='none')
    ax.add_patch(rect)
